...
- `AppLicenses` - A list to define license requirements that must be met before a job can be
  run. See the app_license reference for more details.
...
AppLicenses allow operators to limit the number of simultaneous jobs. More details
on how they work are described in [app_licenses](/nomad/docs/job-specification/app_license)

The `AppLicense` object supports the following keys:

- `License` - Specifies the license name, which must match a variable called
/license/<name>, which must contain a field 'count' with the number of total licenses available.
Licenses which do not match a variable with a count field are considered inifitely available.

- `Count` - Specifies the number of instances of the license to use. A zero or non-existent count
is considered as a value of '1'.
